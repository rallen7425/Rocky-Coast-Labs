...
Gotchas hit during and after the cutover, worth knowing before touching this app's (or any future SPA's) Vercel config:
...
- **Any Vite + React Router (or other client-side-routed) SPA deployed to Vercel needs an explicit SPA-fallback rewrite, or every route except literal `/` 404s in production.** Found 2026-08-24 in `apps/summer-village`: no `vercel.json` existed, so a direct load, refresh, or shared link to `/admin`, `/village`, any event/amenity detail page, etc. hit Vercel's own 404 page — only in-app client-side navigation (which never triggers a real page load) had ever worked, so it went unnoticed through multiple "verified working" checks. This is invisible in local dev (`vite dev`/`vite preview` both have built-in SPA fallback) and easy to miss without deliberately curling routes other than `/` right after a deploy. Fix: a `vercel.json` at the app's Root Directory with `{ "rewrites": [{ "source": "/(.*)", "destination": "/index.html" }] }` — coexists fine with static asset serving (Vercel serves real files before falling through to the rewrite). Check for this file in any other app in this monorepo (or a future one) that uses client-side routing.
